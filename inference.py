...
# Loss Function, Prediction Function, and Metrics (for consistency, not all used in inference)
...
# Optimizer Setup (for consistency, not used in inference)
...
with torch.no_grad():
    ...
    for batch, (input, filenames) in enumerate(inference_loader, 1):
        ...
        for i, file_name in enumerate(filenames):
            # Extract single prediction from batch, move to CPU, remove channel dimension, convert to NumPy
            pred_np = pred[i].cpu().squeeze().numpy()  # shape: (Height, Width)
