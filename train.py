...
# Loss Function, Prediction Function, and Metrics
    ...
    with torch.no_grad():
        ...
        for batch, (input, mask) in enumerate(test_loader, 1):
            ...
            for i in range(pred.size(0)):
                ...
                input_np = input_img[i].cpu().permute(1, 2, 0).numpy()  # (C, H, W) → (H, W, C)
                ...
                # Save mask image (single channel)
                mask_np = mask[i].cpu().squeeze().numpy()  # shape: (H, W)
                mask_np = np.where(mask_np > 0, 255, 0).astype('uint8')  # Convert values greater than 0 to 255, otherwise 0 (binary mask visualization)
                ...
                # Save prediction result (single channel)
                pred_np = pred[i].cpu().squeeze().numpy()  # shape: (H, W)
